...
public class NVBoids : MonoBehaviour
{
    ...
    public float rayonMIN = 5f;
    public float rayonMAX = 10f;
    public bool debug;
    ...
    void BirdsMove()
    {
        ...
        for (int b = 0; b < birdsNum; b++)
        {
            ...
            // birdsTransform[b].transform.position = AdjustPositionToIntersection(birdsTransform[b].transform.position, rayonMIN, rayonMAX);
    ...
    void CreateBird()
    {
        ...
        for (int b = 0; b < birdsNum; b++)
        {
            ...
            birdsTransform[b].localPosition = rdTargetPos[b] = GetRandomPositionInSphereIntersection(rayonMIN, rayonMAX);
    ...
    // Méthode pour obtenir une position aléatoire dans l'intersection de deux sphères
    private Vector3 GetRandomPositionInSphereIntersection(float radiusMin, float radiusMax)
    {
        Vector3 center = Vector3.zero; // Centre commun des deux sphères (peut être ajusté si nécessaire)
        
        // Génère une position aléatoire dans un cube englobant la plus grande sphère
        Vector3 randomPosInCube = new Vector3(
            Random.Range(center.x - radiusMax, center.x + radiusMax),
            Random.Range(center.y - radiusMax, center.y + radiusMax),
            Random.Range(center.z - radiusMax, center.z + radiusMax)
        );

        // Vérifie si la position aléatoire générée se trouve dans le rayon de la sphère la plus petite
        while (randomPosInCube.magnitude < radiusMin)
        {
            // Si elle est dans le rayon de la sphère la plus petite, régénère une nouvelle position
            randomPosInCube = new Vector3(
                Random.Range(center.x - radiusMax, center.x + radiusMax),
                Random.Range(center.y - radiusMax, center.y + radiusMax),
                Random.Range(center.z - radiusMax, center.z + radiusMax)
            );
        ...
        // Normalise la position pour la ramener sur la surface des deux sphères
        Vector3 randomPosInIntersection = randomPosInCube.normalized * Random.Range(radiusMin, radiusMax);

        return randomPosInIntersection;
    }
